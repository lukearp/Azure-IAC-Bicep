Configuration PortalConfiguration
{
    ...
    $rootCertificateData | Out-File -FilePath $RootCertificateLocalFilePath
    ...
    $PortalCertificateLocalFilePath =  (Join-Path 'C:\Temp' $PortalCertificateFileName)
    $TempPortalCertificateLocalFilePath =  (Join-Path 'C:\Temp' 'Temp.pfx')
    #$RootPortalCertificateLocalFilePath =  (Join-Path 'C:\Temp' 'root.cer')
    Out-File -InputObject $PortalInternalCertificateData -FilePath $TempPortalCertificateLocalFilePath
    #Out-File -InputObject $RootInternalCertificateData -FilePath $RootPortalCertificateLocalFilePath
    #Import-Certificate -FilePath $RootPortalCertificateLocalFilePath -CertStoreLocation "Cert:\LocalMachine\Root\"
    $certNoPass = Import-PfxCertificate -Exportable -FilePath $TempPortalCertificateLocalFilePath -CertStoreLocation "Cert:\LocalMachine\My\"
    $certPath = "Cert:\LocalMachine\My\$($certNoPass.Thumbprint)"
    Export-PfxCertificate -FilePath $PortalCertificateLocalFilePath -Password $PortalInternalCertificatePassword.Password -Cert $certPath
