Configuration ServerConfiguration
{
    ...
    $RootcertName = "ESRI-ROOT"
    $ServerCertificateFileName = "internal.pfx"
    $PublicCertificateFileName = "public.pfx"
    $ServerCertificateLocalFilePath =  (Join-Path $env:TEMP $ServerCertificateFileName)
    $PublicCertificateLocalFilePath = (Join-Path $env:TEMP $PublicCertificateFileName)
    $RootCertificateLocalFilePath = (Join-Path $env:TEMP "$($RootcertName).cer")
    $rootcert = New-SelfSignedCertificate -CertStoreLocation cert:\CurrentUser\My -DnsName $RootcertName -KeyUsage CertSign    
    $rootCA = [convert]::tobase64string((get-item "Cert:\CurrentUser\My\$($rootcert.Thumbprint)").RawData) 
    $rootCA | Out-File -FilePath $RootCertificateLocalFilePath
    $publicCert = New-SelfSignedCertificate -certstorelocation cert:\CurrentUser\My -dnsname $ExternalDNSHostName -Signer $rootcert
    $internalCert = New-SelfSignedCertificate -certstorelocation cert:\CurrentUser\My -dnsname $InternalDnsName -Signer $rootcert
    Export-PfxCertificate -Cert $publicCert -Password $ServerInternalCertificatePassword.Password -FilePath $PublicCertificateLocalFilePath -CryptoAlgorithmOption TripleDES_SHA1
    Export-PfxCertificate -Cert $internalCert -Password $ServerInternalCertificatePassword.Password -FilePath $ServerCertificateLocalFilePath -CryptoAlgorithmOption TripleDES_SHA1
    
    Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force
    Set-PSRepository -Name 'PSGallery' -InstallationPolicy Trusted
    Install-Module -Name Az.Accounts -Force -Confirm:$false
    Install-Module -Name Az.KeyVault -Force -Confirm:$false
    Import-Module -Name Az.Accounts
    Import-Module -Name Az.KeyVault    
    Connect-AzAccount -Identity -Environment $azureCloud

    Import-AzKeyVaultCertificate -VaultName $keyVaultName -Name external-cert -FilePath $PublicCertificateLocalFilePath -Password $ServerInternalCertificatePassword.Password
    Import-AzKeyVaultCertificate -VaultName $keyVaultName -Name internal-cert -FilePath $ServerCertificateLocalFilePath -Password $ServerInternalCertificatePassword.Password
    Set-AzKeyVaultSecret -VaultName $keyVaultName -Name "ss-root" -SecretValue $(ConvertTo-SecureString -String $rootCA -AsPlainText -Force) 
    ...
    $ServerCertificateLocalFilePath =  (Join-Path 'C:\Temp' $ServerCertificateFileName)
    $TempServerCertificateLocalFilePath =  (Join-Path 'C:\Temp' 'Temp.pfx')
    $ServerCertificateFileLocation = "\\$($FileShareMachineName)\$FileShareName\Certs\$ServerCertificateFileName"
    Out-File -InputObject $ServerInternalCertificateData -FilePath $TempServerCertificateLocalFilePath 
    $certNoPass = Import-PfxCertificate -Exportable -FilePath $TempServerCertificateLocalFilePath -CertStoreLocation "Cert:\LocalMachine\My\"
    $certPath = "Cert:\LocalMachine\My\$($certNoPass.Thumbprint)"
    Export-PfxCertificate -FilePath $ServerCertificateLocalFilePath -Password $ServerInternalCertificatePassword.Password -Cert $certPath
    if($ServerLicenseFileUrl -and ($ServerLicenseFileUrl.Trim().Length -gt 0)) {
        ...
        Invoke-WebRequest -OutFile (Join-Path $(Get-Location).Path $ServerLicenseFileName) -Uri $ServerLicenseFileUrl -UseBasicParsing -ErrorAction Ignore
    }#>
    if($ServerLicenseFileUrl -and ($ServerLicenseFileUrl.Trim().Length -gt 0)) {
        $ServerLicenseFileName = Get-FileNameFromUrl $ServerLicenseFileUrl
        Invoke-WebRequest -OutFile (Join-Path $(Get-Location).Path $ServerLicenseFileName) -Uri $ServerLicenseFileUrl -UseBasicParsing -ErrorAction Ignore
        ...
        if($HasValidServiceCredential) 
        {
            ...
            if($IsUpdatingCertificates -or ($ServerLicenseFileName -and ($ServerLicenseFileName.Trim().Length -gt 0))){ #On add of new machine or update certificate op
                ...
                ArcGIS_Server_TLS Server_TLS
                {
                    ...
                    DependsOn                  = $ServerDependsOn
                    SslRootOrIntermediate	   = [string]::Concat('[{"Alias":"AppGW-ExternalDNSCerCert","Path":"', $RootCertificateLocalFilePath.Replace('\', '\\'),'"}]') #$null #if($PublicKeySSLCertificateFileName){ [string]::Concat('[{"Alias":"AppGW-ExternalDNSCerCert","Path":"', (Join-Path $(Get-Location).Path $PublicKeySSLCertificateFileName).Replace('\', '\\'),'"}]') }else{$null}
                }
