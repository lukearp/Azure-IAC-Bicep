...
#Get other disks of VMs with shared disks attached
...
foreach ($disk in $disks) {
    ...
    #Check to see if disk is already included as a backup item in a vault
    if ($($instances | ? { $_.property.DataSourceInfo.ResourceId -eq $disk }).count -eq 0) {
        ...
        if($null -eq $vault)
        {
            ...
            #Assigned Disk Reader to Subscription and Disk Snapshot Contributor to Resource Group
            ...
            #Add newly created vault to array so other disks can use the same vault
